...
This tool will get the slot0 for Uniswap V3 pools. The slot0 contains the sqrtPriceX96, tick, observationIndex, observationCardinality, observationCardinalityNext, feeProtocol, and unlocked.
...
The observationIndex is the index of the last oracle observation. The observationCardinality is the current maximum number of observations stored.
The observationCardinalityNext is the next maximum number of observations. The feeProtocol is the current protocol fee as a percentage of the swap fee taken on withdrawal.
